...
<div class="left_box" id="categories_box">
	...
	<ol>
		{section name=i loop=$categories_list->mCategories}
			...
			{if ($categories_list->mCategorySelected == $categories_list->mCategories[i].id)}
				{assign var=selected_c value="class=\"selected\""}
			{/if}
			<li><a {$selected_c} href="{$categories_list->mCategories[i].link|prepare_link:"http"}">&raquo; {$categories_list->mCategories[i].name}</a></li>
		{/section}
	</ol>
</div>
{* End categories list *}
