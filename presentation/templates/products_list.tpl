...
{if $products_list->mrHowManyPages > 1}
	...
	<span class="paging_text">
		...
		{if $products_list->mPreviousLink}
			<a herf="{$products_list->mPreviousLink|prepare_link:"http"}">Previous</a>
		{else}
			Previous
		{/if}
...
{section name=k loop=$products_list->mProducts}
	...
	{if $smarty.section.k.index != 0 && ($smarty.section.k.index + 1) % 2 == 0}
	...
	<p class="{$direction_p}">
		...
		{/if}
		{$products_list->mProducts[k].price}</span>
		{if $products_list->mProducts[k].discounted_price != 0}
