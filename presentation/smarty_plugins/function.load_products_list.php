<?php
// Plugin function inside plugin files must be named: smarty_type_name
...
class ProductsList
{
	...
	public function init()
	{
		...
		elseif (isset($this->_mDepartmentId))
			$this->mProducts = Catalog::GetProductsOnDepartmentDisplay($this->_mDepartmentId, $this->mPageNo, $this->mrHowManyPages);
		// If browsing the first page, get the list of products by calling the GetProductsOnCatalogDisplay business tier method
		else
			$this->mProducts = Catalog::GetProductsOnCatalogDisplay($this->mPageNo, $this->mrHowManyPages);
		...
		if ($this->mrHowManyPages > 1) {
			...
			// If there is no PageNo in the query string then we're on the first page
			if ($pos == FALSE) {
