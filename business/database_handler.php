...
class DatabaseHandler
{
	...
	private static function GetHandler()
	{
		...
		if (!isset(self::$_mHandler)) {
			...
			try {
				...
				self::$_mHandler = new PDO(PDO_DSN, DB_USERNAME, DB_PASSWORD, [PDO::ATTR_PERSISTENT => DB_PERSISTENCY]);
	...
	public static function GetAll($statementHandler, $params = NULL, $fetchStyle = PDO::FETCH_ASSOC)
	{
		...
		// Try to executing the prepared statement received as parameter
		try {
	...
	public static function GetRow($statementHandler, $params = NULL, $fetchStyle = PDO::FETCH_ASSOC)
	{
		...
		try {
			...
			// Close the database handler and trigget an error
			self::Close();
			trigger_error($e->getMessage(), E_USER_ERROR);
		}
